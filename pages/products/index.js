...
const Products = ({ data }) => {
        ...
        <Col key={item.id} sm={22} lg={12} xxl={8}>
          <Link href={`/products/${item._id}`}>
            <div className="product">
              ...
              <p>{item.name}</p>
              <p>
                {item.price}€ {item.price_type === "/kg" ? "/kg" : "/pc"}
              </p>
            </div>
          </Link>
        </Col>
  ...
  return (
    <div className="ourproducts">
      <Hero title="Nos produits" image="ourproducts.webp" />
              ...
              <Col xs={12} xl={6}>
                ...
                >
                  <SubMenu key="sub1" title="Viandes">
                    <Menu.Item
                      key="1"
                      onClick={() => renderFilteredProducts("boeuf")}
                    >
                      <RightOutlined className="iconMenu" />
                      Boeuf
                    </Menu.Item>
                    <Menu.Item
                      key="2"
                      onClick={() => renderFilteredProducts("veau")}
                    >
                      <RightOutlined className="iconMenu" />
                      Veau
                    </Menu.Item>
                    <Menu.Item
                      key="3"
                      onClick={() => renderFilteredProducts("agneau")}
                    >
                      <RightOutlined className="iconMenu" />
                      Agneau
                    </Menu.Item>
                    <Menu.Item
                      key="4"
                      onClick={() => renderFilteredProducts("porc")}
                    >
                      <RightOutlined className="iconMenu" />
                      Porc
                    </Menu.Item>
                    <Menu.Item
                      key="5"
                      onClick={() => renderFilteredProducts("lapin")}
                    >
                      <RightOutlined className="iconMenu" />
                      Lapin
                    </Menu.Item>
                  ...
                  <Menu.Item
                    key="6"
                    onClick={() => renderFilteredProducts("volaille")}
                  >
                    Volaille
                  </Menu.Item>
                  <Menu.Item
                    key="7"
                    onClick={() => renderFilteredProducts("abats")}
                  >
                    Abats
                  </Menu.Item>
                  <Menu.Item
                    key="8"
                    onClick={() => renderFilteredProducts("barbecue")}
                  >
                    Barbecue
                  </Menu.Item>
                  <Menu.Item
                    key="9"
                    onClick={() => renderFilteredProducts("charcuterie")}
                  >
                    Charcuterie
                  </Menu.Item>
                  <Menu.Item
                    key="10"
                    onClick={() => renderFilteredProducts("traiteur")}
                  >
                    Traiteur
                  </Menu.Item>
                  <Menu.Item
                    key="11"
                    onClick={() => renderFilteredProducts("promotions")}
                  >
                    Promotions
                  </Menu.Item>
                  <Menu.Item
                    className="mg-bt-0"
                    key="12"
                    onClick={() => renderFilteredProducts("news")}
                  >
                    Nouveautés
                  </Menu.Item>
                </Menu>
...
async function getProducts() {
  const res = await fetch(process.env.NEXT_PUBLIC_API_PRODUCTS).then(
